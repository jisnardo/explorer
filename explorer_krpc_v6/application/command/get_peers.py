                ...
                for i in self.application_command_get_peers_key:
                    if i[0] == application_command_get_peers_keyword:
                        ...
                        if len(last_query_nodes) == 0:
                            ...
                            for j in last_query_nodes:
                                nodes6_node_id = j[0]
                                nodes6_distance.append(int(info_hash, 16) ^ int(nodes6_node_id, 16))
                            nodes6_distance.sort()
                            for j in last_query_nodes:
                                nodes6_node_id = j[0]
                                nodes6_ip_address = j[1]
                                nodes6_udp_port = j[2]
                                if nodes6_distance[0] == int(info_hash, 16) ^ int(nodes6_node_id, 16):
                                    ...
                                    client.application_client_get_peers_messages_recvfrom.put(
                                        ['application_command_get_peers', info_hash, nodes6_ip_address, nodes6_udp_port, application_command_get_peers_keyword]
                                    )
                                    self.application_command_get_peers_operators.put(
                                        ['append', [application_command_get_peers_keyword, info_hash, nodes6_node_id, nodes6_ip_address, nodes6_udp_port, last_query_nodes, response_nodes, response_values, application_command_commander_keyword, int(time.time())]]
                                    )
            else:
                node_id = application_client_get_peers_messages_send_application_command_get_peers[0]
                nodes6 = application_client_get_peers_messages_send_application_command_get_peers[1]
                            ...
                            if last_query_node_id == node_id:
                                ...
                                if len(values) > 0:
                                    for j in values:
                                        ...
                                        values_tcp_port = j[1]
                                        ip_address_type = IPy.IP(values_ip_address).iptype()[:9]
                                        if ip_address_type == 'ALLOCATED':
                                    ...
                                    for j in nodes6:
                                        ...
                                        if nodes6_distance[0] == int(info_hash, 16) ^ int(nodes6_node_id, 16):
                                            if distance > nodes6_distance[0]:
                                                ...
                                                self.application_command_get_peers_operators.put(
                                                    ['append', [application_command_get_peers_keyword, info_hash, nodes6_node_id, nodes6_ip_address, nodes6_udp_port, nodes6, response_nodes, response_values, application_command_commander_keyword, int(time.time())]]
                                                )
                                            else:
                                                result = {
                                                    'result': response_values,
