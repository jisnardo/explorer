...
class control:
    ...
    def r_client(self, node_id, transaction_id, self_ip_address, self_udp_port, nodes6, values, samples, token, ip_address, udp_port):
        if self_ip_address == '':
            for i in self.driver_control_outbound_nodes_key:
                if i[0] == transaction_id:
                    if i[1] == 'announce_peer':
                        announce_peer_keyword = i[2]
                        self.driver_control_announce_peer_messages.put(
                            [node_id, nodes6, values, samples, token, ip_address, udp_port, announce_peer_keyword]
                        )
                    if i[1] == 'find_node':
                        find_node_keyword = i[2]
                        self.driver_control_find_node_messages.put(
                            [node_id, nodes6, values, samples, token, ip_address, udp_port, find_node_keyword]
                        )
                    if i[1] == 'get_peers':
                        get_peers_keyword = i[2]
                        self.driver_control_get_peers_messages.put(
                            [node_id, nodes6, values, samples, token, ip_address, udp_port, get_peers_keyword]
                        )
                    if i[1] == 'ping':
                        ping_keyword = i[2]
                        self.driver_control_ping_messages.put(
                            [node_id, nodes6, values, samples, token, ip_address, udp_port, ping_keyword]
                        )
                    if i[1] == 'sample_infohashes':
                        sample_infohashes_keyword = i[2]
                        self.driver_control_sample_infohashes_messages.put(
                            [node_id, nodes6, values, samples, token, ip_address, udp_port, sample_infohashes_keyword]
                        )
                    self.driver_control_outbound_nodes_operators.put(
                        ['remove', i]
                    )
        else:
            self_ip_address_type = IPy.IP(self_ip_address).iptype()[:9]
            if self_ip_address_type == 'ALLOCATED':
                if memory.ip_address == self_ip_address:
                    ...
                    if ip_address_type == 'ALLOCATED':
                        ...
                        if wan_ip_address == '':
                            ...
                            ip_address_type = IPy.IP(wan_ip_address).iptype()[:9]
                            if ip_address_type == 'ALLOCATED':
    ...
    def response_find_node(self, transaction_id, nodes6, ip_address, udp_port):
        for i in self.driver_control_inbound_nodes_key:
            if i[0] == transaction_id:
                send().response_find_node(transaction_id, nodes6, ip_address, udp_port)
                self.driver_control_inbound_nodes_operators.put(
                    ['remove', i]
                )
    ...
    def response_get_peers(self, transaction_id, nodes6, values, ip_address, udp_port):
        for i in self.driver_control_inbound_nodes_key:
            if i[0] == transaction_id:
                send().response_get_peers(transaction_id, nodes6, values, ip_address, udp_port)
                self.driver_control_inbound_nodes_operators.put(
                    ['remove', i]
                )
    ...
    def response_sample_infohashes(self, transaction_id, nodes6, number, samples, ip_address, udp_port):
        for i in self.driver_control_inbound_nodes_key:
            if i[0] == transaction_id:
                send().response_sample_infohashes(transaction_id, nodes6, number, samples, ip_address, udp_port)
                self.driver_control_inbound_nodes_operators.put(
                    ['remove', i]
                )

    def start_control(self):
        explorer_krpc_v6_driver_control_check_driver_inbound_nodes_thread = threading.Thread(target = self.__check_driver_inbound_nodes)
        explorer_krpc_v6_driver_control_check_driver_inbound_nodes_thread.setDaemon(True)
        explorer_krpc_v6_driver_control_check_driver_inbound_nodes_thread.start()
        explorer_krpc_v6_driver_control_check_driver_outbound_nodes_thread = threading.Thread(target = self.__check_driver_outbound_nodes)
        explorer_krpc_v6_driver_control_check_driver_outbound_nodes_thread.setDaemon(True)
        explorer_krpc_v6_driver_control_check_driver_outbound_nodes_thread.start()
        explorer_krpc_v6_driver_control_driver_inbound_nodes_operators_thread = threading.Thread(target = self.__driver_inbound_nodes_operators)
        explorer_krpc_v6_driver_control_driver_inbound_nodes_operators_thread.setDaemon(True)
        explorer_krpc_v6_driver_control_driver_inbound_nodes_operators_thread.start()
        explorer_krpc_v6_driver_control_driver_outbound_nodes_operators_thread = threading.Thread(target = self.__driver_outbound_nodes_operators)
        explorer_krpc_v6_driver_control_driver_outbound_nodes_operators_thread.setDaemon(True)
        explorer_krpc_v6_driver_control_driver_outbound_nodes_operators_thread.start()