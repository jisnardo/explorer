...
class krpc_server:
    ...
    driver_interface_socket_server = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    ...
    def bind(self, ip_address, udp_port):
        ...
        explorer_krpc_v4_driver_interface_recvfrom_thread = threading.Thread(target = self.__recvfrom)
        explorer_krpc_v4_driver_interface_recvfrom_thread.setDaemon(True)
        explorer_krpc_v4_driver_interface_recvfrom_thread.start()
        explorer_krpc_v4_driver_interface_send_thread = threading.Thread(target = self.__send)
        explorer_krpc_v4_driver_interface_send_thread.setDaemon(True)
        explorer_krpc_v4_driver_interface_send_thread.start()