...
class send:
    ...
    def response_find_node(self, transaction_id, nodes, ip_address, udp_port):
        new_nodes = bytes()
        for i in nodes:
            nodes_node_id = bytes.fromhex(i[0])
            nodes_ip_address = socket.inet_pton(socket.AF_INET, i[1])
            nodes_udp_port = struct.pack('!H', i[2])
            new_nodes = new_nodes + nodes_node_id + nodes_ip_address + nodes_udp_port
        message = krpc_response_messages().find_node(transaction_id, bytes.fromhex(memory.node_id), new_nodes, ip_address, udp_port)
        krpc_server.driver_interface_send_messages.put(
            [message, ip_address, udp_port]
        )
    ...
    def response_get_peers(self, transaction_id, nodes, values, ip_address, udp_port):
        ...
        new_values = list()
        for i in values:
            values_ip_address = socket.inet_pton(socket.AF_INET, i[0])
            values_tcp_port = struct.pack('!H', i[1])
            new_values.append(values_ip_address + values_tcp_port)
        message = krpc_response_messages().get_peers(transaction_id, bytes.fromhex(memory.node_id), new_nodes, new_values, memory.token, ip_address, udp_port)
        krpc_server.driver_interface_send_messages.put(
            [message, ip_address, udp_port]
        )
    ...
    def response_sample_infohashes(self, transaction_id, nodes, number, samples, ip_address, udp_port):
        ...
        message = krpc_response_messages().ping(transaction_id, bytes.fromhex(memory.node_id), new_nodes, number, new_samples, ip_address, udp_port)
        krpc_server.driver_interface_send_messages.put(
            [message, ip_address, udp_port]
        )