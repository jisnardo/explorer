                    ...
                    if 'm' in peer_extended_response_message['type_20']['payload']:
                        if 'ut_metadata' in peer_extended_response_message['type_20']['payload']['m']:
                            ...
                            if 'metadata_size' in peer_extended_response_message['type_20']['payload']:
                                ...
                                ipv4_address = ''
                                ipv4_udp_port = 0
                                if 'ipv4' in peer_extended_response_message['type_20']['payload']:
                                    ipv4_address = socket.inet_ntop(socket.AF_INET, peer_extended_response_message['type_20']['payload']['ipv4'])
                                    ip_address_type = IPy.IP(ipv4_address).iptype()
                                    if not ip_address_type == 'PUBLIC':
                                        ipv4_address = ''
                                if 'p' in peer_extended_response_message['type_20']['payload']:
                                    ipv4_udp_port = peer_extended_response_message['type_20']['payload']['p']
                                    if not 1 <= ipv4_udp_port <= 65535:
                                        ipv4_udp_port = 0
                                ...
                                if 'type_0' in peer_extended_handshake_response_message:
                                    return ut_metadata, metadata_size, ipv4_address, ipv4_udp_port
                        ...
                        if 'm' in peer_extended_handshake_response_message['type_20']['payload']:
                            if 'ut_metadata' in peer_extended_handshake_response_message['type_20']['payload']['m']:
                                ...
                                if 'metadata_size' in peer_extended_handshake_response_message['type_20']['payload']:
                                    ...
                                    ipv4_address = ''
                                    ipv4_udp_port = 0
                                    if 'ipv4' in peer_extended_handshake_response_message['type_20']['payload']:
                                        ipv4_address = socket.inet_ntop(socket.AF_INET, peer_extended_handshake_response_message['type_20']['payload']['ipv4'])
                                        ip_address_type = IPy.IP(ipv4_address).iptype()
                                        if not ip_address_type == 'PUBLIC':
                                            ipv4_address = ''
                                    if 'p' in peer_extended_handshake_response_message['type_20']['payload']:
                                        ipv4_udp_port = peer_extended_handshake_response_message['type_20']['payload']['p']
                                        if not 1 <= ipv4_udp_port <= 65535:
                                            ipv4_udp_port = 0
                                    return ut_metadata, metadata_size, ipv4_address, ipv4_udp_port
                                else:
